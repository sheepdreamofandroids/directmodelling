...
public class CalculatorTest extends DirectTestBase {
	...
	public void testCalculator() {
		...
		Assert.assertTrue(calculator.calculation.get() instanceof DoubleVar);
		final DoubleValue.Mutable firstNumber = (DoubleValue.Mutable) calculator.calculation
				.get();
		Assert.assertEquals(0, firstNumber.getAsDouble(), 0);
		Assert.assertEquals(0, calculator.calculation.get().getAsDouble(), 0);

		firstNumber.set(12.3);
		Assert.assertEquals(12.3, firstNumber.getAsDouble(), 0);
		Assert.assertEquals(12.3, calculator.calculation.get().getAsDouble(), 0);

		calculator.plus().run();
		Assert.assertTrue(calculator.calculation.get() instanceof FunctionApplication);
		...
		Assert.assertEquals(12.3, calculator.calculation.get().getAsDouble(), E);

		secondNumber.set(-5.5);
		Assert.assertEquals(6.8, calculator.calculation.get().getAsDouble(), E);
	}
}
